...
// SHARED ANALYSIS ENGINE — single source of truth.
// ANALYSIS_SYSTEM_INSTRUCTION and RESPONSE_SCHEMA below are the EXACT verbatim
// engine moved out of api/analyze.js (byte-for-byte identical) so both the
// standard pipeline and the parallel news-refined pipeline import ONE copy and
// can never drift. NEWS_REFINEMENT_MODULE is an additive overlay used only by
// the news pipeline; it never alters this base or the schema.

export const ANALYSIS_SYSTEM_INSTRUCTION = `ATHENIX MASTER IMPLEMENTATION PROMPT
...
export const NEWS_REFINEMENT_MODULE = `
=== NEWS-REFINEMENT MODULE (conditional overlay — appended ONLY when a high-impact news event is in-window) ===

CONTEXT: A high-impact, scheduled macro release (e.g. CPI, NFP/Employment Situation, FOMC, PCE, PPI, GDP, Retail Sales) is slated within this trade's horizon. In the Market Story Engine, such a release is the CATALYST for the Manipulation phase: smart money runs a violent liquidity raid (judas swing / stop hunt) to grab resting liquidity BEFORE the real expansion, accompanied by a liquidity vacuum, sharply widened spreads, and slippage.

DIRECTIVE (strictly predictive — one shot, no waiting, no live confirmation): Produce the SAME analysis through ALL existing layers exactly as defined above, with EXACTLY TWO refinements to the resulting trade setup, and nothing else:

1. ENTRY — relocate the predicted entry to the MANIPULATION TARGET: the liquidity pool / POI (in correct premium/discount alignment, against the immediate move) that the news raid is most likely to drive price INTO before expansion. This is the discounted/premium zone a standard entry would have been stopped out from — here it becomes the entry. Choose it from the liquidity map / POI selection you already computed; do not invent a level.

2. STOP LOSS — place it JUST BEYOND the predicted swept extreme of that manipulation zone (a few pips past the raid's wick / beyond the protected level), with only a small buffer. Keep it TIGHT to maximize risk-reward — do NOT widen the stop to "survive" the event.

UNCHANGED: The take-profits / draw on liquidity remain EXACTLY the destination you computed for the standard read — do not move any target. All structural definitions, confluence scoring, quality scoring, probabilities, sessions, and the output schema remain exactly as specified. Only the setup's entry and stop change.

QUALITY: If you cannot identify a high-confidence manipulation entry under these conditions, return final_decision = "no_trade" (decline — never wait). Annotate volatility_context that spreads/liquidity are abnormal around the event, which is why the stop sits just beyond the manipulation extreme. Explain the manipulation-entry logic in the reasoning fields.

Output in the SAME response schema as always (signal / impulse_setup / corrective_setup carry the refined entry and stop; targets unchanged).
`;

export function buildNewsSystemInstruction(eventSummary) {
  const evt = eventSummary ? ('\n\nIN-WINDOW HIGH-IMPACT EVENT(S): ' + eventSummary) : '';
  return ANALYSIS_SYSTEM_INSTRUCTION + '\n\n' + NEWS_REFINEMENT_MODULE + evt;
}
