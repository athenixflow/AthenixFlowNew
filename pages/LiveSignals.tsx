              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-brand-charcoal text-white rounded-xl flex items-center justify-center font-black text-lg">
                      {safeRender(signal.instrument, "").substring(0, 2)}
                    </div>
                    <div>
                      <h4 className="text-lg font-black text-brand-charcoal uppercase tracking-tighter">{safeRender(signal.instrument)}</h4>
                      <p className="text-[10px] text-brand-muted font-bold uppercase tracking-widest">
                        {safeRender(signal.execution_mode, "").replace('_', ' ')} • {safeRender(signal.timeframe)}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="p-3 bg-brand-sage/5 rounded-xl">
                    ...
                    <p className="text-xs font-bold text-brand-charcoal">{safeRender(signal.signal?.entry_price, '---')}</p>
                  </div>
                  <div className="p-3 bg-brand-sage/5 rounded-xl">
                    <p className="text-[8px] font-black text-brand-muted uppercase tracking-widest mb-1">Stop Loss</p>
                    <p className="text-xs font-bold text-brand-error">{safeRender(signal.signal?.stop_loss, '---')}</p>
                  </div>
                  <div className="p-3 bg-brand-sage/5 rounded-xl">
                    <p className="text-[8px] font-black text-brand-muted uppercase tracking-widest mb-1">Target</p>
                    <p className="text-xs font-bold text-brand-success">{safeRender(signal.signal?.take_profits?.[0]?.price, '---')}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-brand-sage/10">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="text-[10px] font-black text-brand-gold uppercase tracking-widest">Quality: {safeRender(signal.quality_score, "0")}%</span>
                  </div>
                </div>
              </div>
            </div>
